...
function toNav(grade, akasataNames) {
  ...
  for (var i=0; i<akasataNames.length; i++) {
    var url = '/homonym-ja/' + dirNames[grade-1] + '/' + akasataNames[i] + '/';
...
var homonyms = [];
readEachLineSync('homonym.csv', 'utf8', (line) => {
  var arr = line.split(',');
  var yomi = arr[0];
...
for (var i=0; i<index.length-1; i++) {
  for (var j=index[i]; j<index[i+1]; j++) {
    var yomi = homonyms[j][0];
    var words = homonyms[j][1];
...
for (var g=1; g<learnedKanjis.length + 1; g++) {
  ...
  for (var i=0; i<akasataNames.length; i++) {
    ...
    var html = ejs.render(template, {
      grade:g, gradeName:gradeNames[g-1], homonyms:data[g][i], akasataName:akasataNames[i],
